/**
 * Claude Code — built-in "deep-research" Workflow (source)
 * ========================================================
 *
 * WHAT THIS IS
 *   The complete source of the built-in `deep-research` named workflow that
 *   ships inside Claude Code. It is invoked via `/deep-research` (or
 *   `Workflow({ name: "deep-research", args: "<question>" })`) and runs a
 *   fan-out research harness: Scope -> Search -> Fetch -> 3-vote adversarial
 *   Verify -> Synthesize, using the WebSearch / WebFetch tools.
 *
 * WHERE IT CAME FROM
 *   This workflow is NOT a user-editable file on disk. It is compiled into the
 *   Claude Code native executable and registered at runtime by
 *   `initBundledWorkflows()`. The script body below is stored as an embedded
 *   string literal inside the binary.
 *
 *   Extracted from:
 *     ~/.local/share/claude/versions/2.1.162   (Mach-O 64-bit arm64 executable)
 *     which is the binary that `~/.local/bin/claude` resolves to.
 *
 *   Located in the binary at byte offset ~205,776,800–205,795,200, identified
 *   by the literals "deep-research" and "Deep research harness". Recovered by
 *   carving that byte range, stripping NUL padding, and decoding the embedded
 *   \uXXXX escapes back to UTF-8. The `meta` block values (name / description /
 *   whenToUse / phases) were substituted from their corresponding string
 *   constants in the same binary (`_0K`, `q0K`, `K0K`, `O0K`).
 *
 *   Claude Code version: 2.1.162
 *
 * NOTE
 *   This is reverse-engineered from Anthropic's proprietary Claude Code binary,
 *   reproduced here for study/reference. All rights to the original code belong
 *   to Anthropic. An in-code comment reveals its lineage:
